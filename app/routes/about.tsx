  ...
  const portfolioData: PortfolioData = {
    ...
    specialties: [
      ...
      },
      {
        ...
        icon: '🔄',
        description:
          'Docker, CI/CD 파이프라인 구축, AWS 서비스 활용 경험이 있어 개발부터 배포까지 전체 과정을 다룰 수 있습니다.',
      },
      {
        id: 'spec-4',
        name: 'UI/UX 디자인',
...
export default function About() {
  ...
  return (
    <div ref={containerRef} className="bg-white dark:bg-gray-900" style={getSectionStyle()}>
      {/* 헤더 섹션 */}
      <motion.div
        className="relative h-[70vh] flex flex-col items-center justify-center text-center px-4"
        style={{ opacity: headerOpacity, scale: headerScale }}
      >
        <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-blue-500 mb-6">
          <img
            src={portfolioData.developer.avatar}
      ...
      <section
        ref={timelineRef}
        className="py-20 px-4 md:px-10 max-w-5xl mx-auto bg-gray-100 dark:bg-gray-800/50"
        id="timeline"
      >
        <motion.div
          initial={{ opacity: 0 }}
      ...
      <section
        ref={educationRef}
        className="py-20 px-4 md:px-10 max-w-6xl mx-auto bg-gray-100 dark:bg-gray-800/50"
        id="education"
      >
        <motion.div
          initial={{ opacity: 0 }}
      ...
      <section
        ref={specialtiesRef}
        className="py-20 px-4 md:px-10 max-w-4xl mx-auto bg-gray-100 dark:bg-gray-800/50"
        id="specialties"
      >
        <motion.div
          initial={{ opacity: 0 }}
        ...
        <motion.div
          ...
          style={{ height: selectedSpecialty ? 'auto' : '0px' }}
          ...
          {selectedSpecialty && portfolioData.specialties.find(s => s.id === selectedSpecialty) && (
            ...
            >
              <h3 className="text-xl font-bold text-pink-600 dark:text-pink-400 mb-3">
                {portfolioData.specialties.find(s => s.id === selectedSpecialty)?.name}
              </h3>
              <p className="text-gray-700 dark:text-gray-300">
                {portfolioData.specialties.find(s => s.id === selectedSpecialty)?.description}
              </p>
            </motion.div>
          )}
        </motion.div>
