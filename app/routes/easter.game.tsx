...
export default function GameRoute() {
	return (
		<div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-white p-4">
			...
			<p className="mb-6 text-center max-w-md">축하합니다! 비밀 미니게임을 발견하셨습니다. 방향키로 플레이어를 움직여 아이템을 수집하세요!</p>
...
function Game() {
	...
	useEffect(() => {
		...
		const keyDownHandler = (e: KeyboardEvent) => {
			...
			setGameState((prev) => {
				const newPlayerX =
					key === "ArrowLeft" ? Math.max(prev.player.size / 2, prev.player.x - 15) : key === "ArrowRight" ? Math.min(canvas.width - prev.player.size / 2, prev.player.x + 15) : prev.player.x;

				const newPlayerY =
					key === "ArrowUp" ? Math.max(prev.player.size / 2, prev.player.y - 15) : key === "ArrowDown" ? Math.min(canvas.height - prev.player.size / 2, prev.player.y + 15) : prev.player.y;
	...
	return (
		<div className="flex flex-col items-center">
			...
			{!gameState.gameStarted && (
				<button className="mt-4 px-6 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-md transition-colors" onClick={initGame}>
					게임 시작
				</button>
			)}
			{gameState.gameOver && (
